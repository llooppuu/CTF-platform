  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (result.success) {
        ...
        setErrors(result.errors?.length ? result.errors : ["Login failed"]);
      }
    } catch (error) {
        ...
        <div className="col-lg-5">
          <div className="card border-0 shadow-sm"><div className="card-body p-4">
            ...
            <form onSubmit={handleSubmit} className="d-grid gap-3">
              <div><label className="form-label">Username or email</label><input className="form-control" value={form.name} onChange={(event) => setForm((previous) => ({ ...previous, name: event.target.value }))} /></div>
              ...
              <button className="btn btn-dark" disabled={submitting}>{submitting ? "Logging in…" : "Login"}</button>
            </form>
          </div></div>
        </div>
      </div>
